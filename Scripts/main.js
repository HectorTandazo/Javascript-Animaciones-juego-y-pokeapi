...
var Level = Base.extend({
	...
	load: function (level) {
		...
		this.timeLeft = 400; // Tiempo del nivel
		var data = level.data;
	...
	playSound: function (label) {
		if (this.soundManager)
			this.soundManager.play(label);
	},
	playMusic: function (label) {
		if (this.soundManager)
			this.soundManager.playMusic(label);
	},
	reset: function () {
		this.active = false;
		this.world.empty();
	...
	updateUI: function() {
		this.world.parent().children('#scoreNumber').text(this.score || 0);
		this.world.parent().children('#timeNumber').text(this.timeLeft || 0);
	},
	...
	startTimer: function() {
		...
		this.timeInterval = setInterval(function() {
			if (self.timeLeft > 0) {
				self.timeLeft--;
				self.updateUI();
				if (self.timeLeft <= 100 && self.timeLeft % 50 === 0) {
					self.playSound('time-warning');
				}
				if (self.timeLeft === 0) {
					// Tiempo agotado
					for (var i = self.figures.length; i--;) {
						if (self.figures[i] instanceof Mario) {
							self.figures[i].die();
							break;
						}
					}
...
var Figure = Base.extend({
	...
	// Mejorar detección de colisiones más precisa
	getBounds: function() {
		return {
			left: this.x,
			right: this.x + 32,
			top: this.y + this.state * 32,
			bottom: this.y
		};
	},
	...
	move: function () {
		...
		// Colisión horizontal mejorada
		for (var i = 0; i < d; i++) {
			if (this.collides(t + dx, t + dx, js, je, b)) {
				vx = 0;
				x = t * 32 + 15 * dx;
				// Sonido de colisión para Mario
				if (this instanceof Mario && Math.abs(this.vx) > 3) {
					this.level.playSound('block-bump');
				}
				break;
			}
		...
		for (var i = 0; i < d; i++) {
			if (this.collides(is, ie, t - dy, t - dy, b)) {
				onground = dy < 0;
				// Sonido de aterrizaje para Mario
				if (this instanceof Mario && dy < 0 && Math.abs(this.vy) > 5) {
					this.level.playSound('goomba-stomp');
				}
				vy = 0;
...
var Coin = Item.extend({
	...
	activate: function (from) {
		if (!this.activated) {
			...
			from.addCoin();
			// Efecto de partículas para monedas
			if (this.level.particles) {
				this.level.particles.createCoinEffect(this.x + 16, this.y + 16);
			}
			this.remove();
		}
		this._super(from);
...
var Star = ItemFigure.extend({
	...
	release: function () {
		this.taken = 4;
		this.active = true;
		this.level.playSound('powerup-appears');
		this.view.show();
...
var Mario = Hero.extend({
	...
	input: function (keys) {
		...
		if (!this.crouching) {
			...
			if (keys.right || keys.left) {
				...
				// Mejorar control de dirección
				this.direction = keys.left ? directions.left : directions.right;
			} else {
				// Fricción mejorada
				this.vx *= 0.8;
	...
	walk: function (reverse, fast) {
		var targetSpeed = constants.walking_v * (fast ? 1.6 : 1) * (reverse ? -1 : 1);
		// Aceleración gradual mejorada
		var acceleration = fast ? 0.6 : 0.4;
		this.vx += (targetSpeed - this.vx) * acceleration;
		// Limitar velocidad máxima
		var maxSpeed = constants.walking_v * (fast ? 1.8 : 1.1);
	...
	jump: function () {
		...
		// Salto variable según velocidad
		var jumpPower = constants.jumping_v;
		if (this.fast && Math.abs(this.vx) > constants.walking_v) {
			jumpPower *= 1.1; // Salto más alto cuando corre
		}
		this.vy = jumpPower;
		// Efecto de partículas al saltar
		if (this.level.particles) {
			this.level.particles.createJumpEffect(this.x, this.y);
		}
	},
	move: function () {
		this.input(keys);
	...
	die: function () {
		...
		this.level.soundManager.stopMusic();
...
var Enemy = Figure.extend({
	...
	hit: function (opponent) {
		...
		if (opponent instanceof Mario) {
			// Detección correcta de pisoteo: Mario debe estar cayendo y por encima del enemigo
			if (opponent.vy < 0 && opponent.y > this.y + this.state * 16) {
				// Mario pisa al enemigo
				opponent.setVelocity(opponent.vx, constants.bounce);
				this.hurt(opponent);
				this.level.addScore(100);
			} else {
				// Mario toca al enemigo por los lados
				opponent.hurt(this);
			}
		}
	},
...
var Gumpa = Enemy.extend({
	...
	die: function () {
		...
		if (this.death_mode === death_modes.normal) {
			...
			if (this.level.particles) {
				this.level.particles.createDeathEffect(this.x + 16, this.y + 16);
			}
...
var TurtleShell = Enemy.extend({
	...
	hit: function (opponent) {
		...
		if (this.vx) {
			...
			// Caparazón quieto - Mario puede patearlo
			if (opponent instanceof Mario) {
...
var SpikedTurtle = Enemy.extend({
	...
	hit: function (opponent) {
		...
		if (opponent instanceof Mario) {
			// Tortuga con pinchos siempre daña a Mario
			opponent.hurt(this);
		}
	},
}, 'spikedturtle');
...
var Plant = Enemy.extend({
	...
	hit: function (opponent) {
		...
		if (opponent instanceof Mario) {
			// Las plantas siempre dañan a Mario
			opponent.hurt(this);
		}
	},
});
 ...
 * -------------------------------------------
 */
// Inicialización movida al HTML para tener acceso global